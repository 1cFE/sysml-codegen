...
- [P-003 — No workarounds to accept bad models](P-003-no-workarounds-for-bad-models.md) — `[OWNER-VERBATIM, 2026-08-16]`. Ill-formed model text is refused with a diagnostic, never resolved by invented behavior; we follow what KerML, SysML v2, and SysIDE support. First application covers every definition-owned lineage miss on the exact-address route; the owner-authorized close records the final production identity and the transferred diagnostic follow-up.
- [P-004 — What this product is: parse the models, walk the AST, reconstruct the math, write it into TEAx Python](P-004-product-identity-parse-walk-emit.md) — `[OWNER-VERBATIM, 2026-08-16]`. The product is those three steps; any manual fallback or workaround for an unresolved reference is a massive, disgusting smell
