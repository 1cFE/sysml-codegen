...
**Checkpoint 0**: All baseline snapshots captured. Current tests pass (454+). Harness ready.

---

## Phase 1: Foundation Components (no pipeline dependencies)

**Goal**: Validate and lock down the lowest-level building blocks.
...
### 1.3 — Expression Compiler Conformance (C04)
...
### 1.4 — Computed Attribute Classification Conformance (C05)
...
**Checkpoint 1**: Foundation locked. All naming, data model, and expression compilation
requirements verified. ~30-50 new conformance tests.
...
### 6.3 — Module Registry Generator (C24)
...
### 6.5 — Type Mapping Consolidation (X01)
...
**Final Checkpoint**: Full test suite green (454+ existing + ~150-200 new conformance tests).
All baselines match. All 161 requirements have at least one test. Codebase matches target
...
| 1 | Foundation | Data models, naming, expressions locked | ~40 |
...
| 6 | Generation | All generators validated against graph | ~30 |
| 7 | Refactor | Structural cleanup, dead code gone, PipelineModule expanded (C26) | ~10 |

**Total**: ~240 new conformance tests on top of existing 454.
