...
35. (amended 2026-08-14, Item 7 correction step 5, per disposition 4) Live and supported snapshot
    routes agree semantically — decisions, diagnostics, retained producers, graph/catalog values,
    fingerprints — and byte-for-byte on generated output after the defined normalization of
    permitted provenance metadata. The defined normalization is the shared portable source-referent
    rendering (`root-N/`, NFC segments, percent-encoded), applied by each route at elaboration from
    its own evidence: the caller's model roots live, the sealed admission manifest during capture.
    After it no permitted route-dependent metadata remains, so generated-byte equality is literal.
