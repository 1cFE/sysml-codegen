...
It is frozen deliberately. The 14 fixture roots this item promotes are frozen separately in
`promoted_roots.json`, captured in a `promoted` section of the ledger, and never enter the corpus
counts. Later work can add fixtures without silently changing this item's evidence population.

## `before.json` — the pre-repair corpus ledger

```bash
uv run python .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/corpus_compare.py \
  --output .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/before.json
```
...
uv run python scripts/assess_v6_snapshot_churn.py \
  --output .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/before-snapshot-inventory.json
```
...
```bash
uv run python scripts/assess_v6_snapshot_churn.py \
  --output .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/phase4-snapshot-assessment.json
```
...
```bash
uv run python .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/adjudicate.py \
  .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/before.json \
  .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/after.json \
  > .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/adjudication-diff.txt
```
...
field. No license needed. **20 changed rows, 0 structural problems.**

### `adjudication.md` — the adjudication itself

Every one of the 20 changed rows ruled on with topology, exact before/after identity, and reasoning;
...
```bash
uv run python .project/completed/20260816_qualified-reference-occurrence-anchoring/verification/\
absent_leaf_census.py --output .project/completed/20260816_qualified-reference-occurrence-anchoring/\
...
element index did not hold the leaf. It now refuses by name. The retained result finds **0 absent
leaves among 770 observed resolver calls** from 139 complete roots and the observed prefix of one
partial root. Fifteen roots retain unmeasured population, so the census does not establish
whole-corpus authored reachability. Its population is wider than the ledger's because the refusal
fires before an owner can be classified, so owner kind cannot be part of the question.
...
15 roots hold an unmeasured or partially measured population. A refused root with no observed call
has numeric zeroes, but its `none` state keeps those zeroes out of complete-population evidence.
