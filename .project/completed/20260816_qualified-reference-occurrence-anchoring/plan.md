...
**Status:** Certified — independent re-audit findings reconciled and final checks passed (2026-08-16)
**Owner:** Reid W
...
- **Design:** `.project/completed/20260816_qualified-reference-occurrence-anchoring/design.md`
...
- [x] Create
  `.project/completed/20260816_qualified-reference-occurrence-anchoring/spike/deep-override-authorability/`
...
- [x] Create
  `.project/completed/20260816_qualified-reference-occurrence-anchoring/verification/corpus_compare.py` and a
...
- [x] Reconcile every spec success criterion with one retained test or evidence row. SC8 has an
  authored discriminating fixture; the deep-override lane remains explicitly unevidenced under the
  owner's accepted bound rather than being represented as exercised.
...
**What We Knew After This Phase:** The exercised resolver behavior and edge/refusal adjudication
passed. Certification was still open at this point; Phases 6–7 repaired corpus identity/census
evidence, and the final reconciliation records the owner's deep-override disposition.
...
- **`adjudication.md` — the deliverable.** All 19 Phase-5 changed rows were ruled on; Phase 6 added
  and adjudicated row 20. The final document reconciles all 14 success criteria and the bounded
  documentation check.
...
census did not establish whole-corpus authored reachability because refused roots were not counted.
Phase 7 repairs the measurement: it finds zero absent leaves among 770 observed calls and names 15
roots with residual unmeasured population. The boundary-only test stands as a fail-closed state
test, not as authorability evidence.
...
- Every root now carries a `measurement` of `complete` (run returned), `partial` (run raised after
  the branch had already run), or `none` (no resolver-boundary call was observed before refusal),
  each non-complete row keeping its reason. Totals carry `residual_unmeasured_roots` and a
  `population_claim` string, so nothing reads a refused root as zero leaves and zero absent.
...
**Re-verification:** **Closed after claim correction.** The mechanism and regenerated totals pass.
Every retained claim now says 0 absent among 770 observed calls and preserves the 15-root residual;
none infers whole-corpus authorability. The `none` definition now says no resolver-boundary call was
observed before refusal. See `audit.md`, “Phase 7 re-verification.”
...
(2026-08-16, Phase 7) → Phase 7 mechanism and claims verified; **finding 5 closed** → final
reconciliation closes or disposes findings 2, 3, 4, and 7; **Certified 2026-08-16**.
**Next Step:** Close and archive the item.
