...
**Status:** Certified 2026-08-16. SC12 is
verified: 139 elaborated roots carry unchanged identity blocks, 15 refused roots carry no graph
identity, and the comparator rejects an absent block. Finding 5's census was repaired on 2026-08-16:
it now counts at the resolver boundary, so a root that later refuses still reports the leaves it
saw. Every root carries an explicit complete / partial / unmeasured measurement. Re-derived totals:
**154 roots —
139 complete, 1 partial, 14 unmeasured; 770 one-segment leaves observed, 0 absent from the element
index.** 15 roots hold residual unmeasured population. The JSON itself makes no whole-population
claim. All finding-5 artifacts now state the bounded result consistently: 0 absent among 770
observed resolver calls; whole-corpus authored reachability is not established. SC1 is met under
the owner's named deep-override evidence bound, and the close-time SC14 documentation check passes.
See `audit.md` and the latest `product-lens.md` block.
**Owner:** Reid W
...
- [x] At close, the active self-binding spec is checked against the landed behavior and still names
...
- **Spec review:**
  `.project/completed/20260816_qualified-reference-occurrence-anchoring/spec-review.md` — `Revise`; all fifteen
...
- **Design:** `.project/completed/20260816_qualified-reference-occurrence-anchoring/design.md` (to be created)
...
`.project/completed/20260816_qualified-reference-occurrence-anchoring/spec-review.md` is accepted. How each was
resolved:
