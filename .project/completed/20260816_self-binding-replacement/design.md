...
**Status:** Implemented (rev 6 — independent re-audit pending)
**Owner:** Reid W
**Stage:** `design` (orchestrated run; briefs `04-design.md`, `05-design-review.md`)
**Created:** 2026-08-15 · **Revised:** 2026-08-16 (rev 6)
...
The distinction remains important. The delivered code still sends definition-owned leaves through
`_resolve_leaf`; usage-owned leaves do not use that route. `[OWNER 2026-08-16]` has now ruled the
lineage-miss descendant search inside `_resolve_leaf` unsupported: SysIDE's resolved structure is
authoritative, and codegen must refuse rather than invent an occurrence through position. Local
definition-owned projection through the consumer's own occurrence lineage remains in scope. The
owner-verbatim source, bound, and implemented vehicle are recorded at
`[DEF-OWNED-SIDEWAYS-REACH]`.
...
`_resolve_leaf`. For a definition-owned feature leaf, the target design may map the feature through
the consumer's own occurrence lineage. If that mapping misses, it emits a named refusal; it does
not search descendants or sibling subtrees for a unique candidate. Calculation outputs may still
select a producing calculation node through their separate route. The delivered code has not yet
made this change; `[DEF-OWNED-SIDEWAYS-REACH]` owns it.
...
3. **By owner qualification → D-6, split by resolved owner class.** A usage-owned leaf anchors on
   the exact usage SysIDE resolved. A definition-owned leaf may map through the consumer's own
   occurrence lineage, but a lineage miss refuses instead of searching descendants. The 13
   published examples are usage-owned and are governed by D11. The definition-owned refusal is
   implemented and pinned at one occurrence above, two occurrences above, and sibling scope.
...
- `docs/patterns/plant-idiom.md` — the authoritative copy. Self-binding section reorganized by
  ...
  anchoring and the definition-owned lineage-miss refusal stated.
...
## Appendix B — Measured behavior and the ruled F-4 target
...
| `in availability = 'Plant'::availability` (**definition-owned leaf**) | above the def, 1 occurrence | refused, `SI_OCCURRENCE_MISSING` |
| `in availability = 'Plant'::availability` (**definition-owned leaf**) | above the def, 2 occurrences | refused, `SI_OCCURRENCE_MISSING` |
| `in unit_cost = 'Unit'::cost` (**definition-owned leaf**) | no local occurrence, 1 in a sibling subtree | refused, `SI_OCCURRENCE_MISSING` (former F-4, reproduced at `0f89673`) |
...
## Revision Record — rev 6

The owner-ruled definition-owned lineage-miss fallback is removed. Authored qualification now
requires a matching feature slot on the consumer's occurrence lineage and refuses by the existing
`SI_OCCURRENCE_MISSING` diagnostic when absent. The resolver carries that qualification fact from
extraction to the leaf route; exact usage-owner anchoring, local definition-owned lineage mapping,
bare references, and explicit occurrence paths are unchanged. The authoritative guide and retained
one-above, two-above, and sibling fixtures now state and prove the refusal. The positional fixture
guard and injective cross-tree oracle also close the re-audit's two assurance gaps.
...
|---|---|
| Stale D-6 teaching / D11 | **Accepted in rev 4, superseded in rev 5.** The owner later ruled the definition-owned lineage-miss descendant fallback unsupported; exact usage-owner anchoring and local definition-owned projection remain distinct. |
