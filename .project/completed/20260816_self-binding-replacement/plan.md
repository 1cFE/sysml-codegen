...
**Status:** Closed by owner direction — testing/developer-tooling edge accepted as risk
**Created:** 2026-08-16
...
- [x] Add `tests/conformance/test_definition_owned_reference_positions.py`:
  ...
  explicitly labelled as positional fallback, not checked author intent. *(Post-plan owner ruling,
  2026-08-16: that lineage-miss fallback is unsupported. `[DEF-OWNED-SIDEWAYS-REACH]` owns the
  test flip and implementation; this completed box records the original evidence.)*
- [x] Implement `[DEF-OWNED-SIDEWAYS-REACH]`: carry whether a one-segment reference was authored
  with qualification into `_resolve_leaf`; for a qualified definition-owned leaf, map only through
  the consumer lineage and otherwise refuse with `SI_OCCURRENCE_MISSING`. Preserve exact
  usage-owner anchoring, local definition-owned lineage mapping, bare references, and explicit
  occurrence paths.
- [x] Promote the one-occurrence-above shape and flip it, the two-occurrences-above shape, and the
  sibling-scope shape to the named refusal. Re-derive affected mixed-consumer fixtures as explicit
  D-7 paths and update the authoritative agentic guide.
...
- [ ] Extend `scripts/make_d5_variant.py` with `--root`, explicit discovery/precondition output, a
  ...
  *(Independent re-audit 2026-08-16: a dangling pre-existing `--scratch` symlink is resolved before
  the existence check, so customer mode returns 0 and creates the referent directory. Reject the
  lexical symlink before resolution and retain a no-mutation falsifier.)*
- [ ] Guard positional fixture mode before `build_variant_tree()` can delete anything. Reject
  absolute or traversal operands, source/target equality or overlap, and an unsafe pre-existing
  target; prove every refusal leaves the fixture source and target bytes unchanged. *(Re-audit
  2026-08-16: the six remediation tests cover only `--root/--scratch`. Positional operands still
  escape `tests/fixtures/` and reach unconditional `shutil.rmtree(target_dir)`.)* *(Completed
  2026-08-16: fixture operands are resolved beneath `tests/fixtures/`; absolute, traversal,
  escaped-symlink, equal, overlapping, missing, and pre-existing targets refuse with exit 1 before
  mutation. The happy new-target route and the lower-level overlap guard are retained. 39 passed.)*
  *(Independent re-audit 2026-08-16: a dangling pre-existing target symlink is resolved before the
  original operand is checked, so positional mode returns 0 and creates the referent directory;
  a symlink loop also escapes as an uncaught exception. Refuse both before mutation.)*
...
- [x] `uv run --extra dev pytest tests/conformance/test_d5_variants.py` — 29 passed (20 existing +
  9 new).
- [ ] Run every hazard case with a before/after tree digest; all refuse and leave the tree unchanged.
  — Each precondition test runs `main()` end to end and asserts exit 1, no scratch tree created,
  and a byte-level tree digest unchanged. *(Independent re-audit 2026-08-16: dangling target
  symlinks are missing from both CLI-mode hazard matrices and currently mutate their referents.)*
...
- [x] Make the retained cross-tree layout mapping injective before constructing comparison
  dictionaries. Prove that colliding primary paths such as `library/x.sysml` and `x.sysml` cannot
  discard an unpaired or different file. *(Re-audit 2026-08-16: a temporary falsifier passed with
  one primary file silently overwritten by the normalized dictionary key.)* *(Completed
  2026-08-16: `_model_files_by_logical_path` asserts key uniqueness before insertion; the retained
  collision falsifier proves `library/x.sysml` plus `x.sysml` refuses. Fusion spine: 10 passed.)*
...
- [ ] Review each repository's final diff against its phase-owned paths. Preserve every unrelated
  ...
  *(Independent re-audit 2026-08-16: the final lineage/path repairs, guide changes, and Fusion
  injectivity guard are newer uncommitted worktree changes, so the recorded review is stale.)*
- [ ] Create path-scoped local commits in agentic-mbse, sysml-codegen, and the isolated fusion-tea
  ...
  **stellarator**: nothing. *(Independent re-audit 2026-08-16: these hashes do not contain the
  final repairs now present in all three worktrees.)*
...
- `tests/conformance/test_definition_owned_reference_positions.py` now has 4 tests: inside-the-def
  two occurrences generate per-occurrence values (0.11/0.99, own-occurrence edges); above-the-def
  with one or two occurrences and sibling scope all refuse with `SI_OCCURRENCE_MISSING`, leaving
  lenient consumers unbound. `[OWNER 2026-08-16]` ruled the lineage-miss fallback unsupported;
  `[DEF-OWNED-SIDEWAYS-REACH]` is implemented in the resolver and authoritative guide.
...
re-audit accepted those four remediations in their intended paths. The two assurance boxes it
opened are now implemented: D5 fixture mode is guarded before deletion, and Fusion Tea's logical
path mapping is injective before comparison dictionaries are built. The owner-ruled F-4 vehicle is
also implemented. Final implementation validation: D5 39 passed; owner-reference battery 52
passed; combined semantic battery 114 passed; Fusion spine 10 passed; agentic focused 31 passed
and full 1835 passed / 1 skipped / 33 deselected. Final codegen full licensed suite: 17 documented
missing-`pandas` environment failures / 2181 passed / 34 skipped / 88 deselected. Changed-file Ruff
and three-repo `git diff --check` are clean.
...
**Status:** Draft → In Progress → Implementation Complete → Audit Needs Work → Implementation Complete → Audit Needs Work → Closed with Owner-Accepted Risk
