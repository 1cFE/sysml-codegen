...
| **F5** — a live gate reads an artifact under `.project/completed/` | Low | **RESOLVED by owner decision [OWNER 2026-08-13]:** the ledger moved (`git mv`, bytes unchanged) to `tests/unit/data/expected-coverage.md` — the expected-value file lives beside the test that reads it, so collection never depends on archive layout. Pointer stub left at the archived path; `test_coverage_ledger_agreement.py` 14/14 at the new path | post-cure |
