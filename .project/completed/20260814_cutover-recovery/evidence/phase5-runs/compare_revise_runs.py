#!/usr/bin/env python3
"""Compare the three REVISE step-7a battery runs field by field.

Reads `revise-runs/run{1,2,3}/` and prints one row per measured field with three
columns. Any field whose three values are not identical is marked DIFFER, which
under the step-7a brief is a rule-10 stop.
...
are printed separately as a measured range.
    ...
    python compare_revise_runs.py > revise-runs/comparison.md
"""
...
def read(run: str, name: str) -> str:
    path = HERE / "revise-runs" / run / name
