...
| 12 | `d38_caret` | graph 1/1/0/0 | graph 2/6/0/0 | M/E/O | expected-fix | Modeled finite `[count]`, where `count = 4`, expands four occurrences. **Shipped-package cells, measured at the recovery Slice 3E authority switch (2026-08-11):** before, the package carried `inputs/design_params.json` with the single key `D38Design__plant__noop__x`; after, it carries `inputs/library_params.json` with that key plus `D38Design__plant__pack__exponent` and four `D38Design__plant__pack__cell[i]__base_cost`. The group name follows the declaration site — the elaborator records `library.sysml`, the legacy deriver attributed it to `design.sysml`. **[AGENT] (orchestrator ruling, 2026-08-11)** classified expected-fix at the switch: string-era attribution is the ratified defect home, and declaration-site semantics are the correct answer. Fixture-internal; flagged to the Phase 5 owner packet. |
...
| 15 | `fusion_tea` | graph 9/31/1/5 | graph 9/27/1/7 | M/E/O/A | expected-fix | Recovery Slice 3D migrated the customer model's fifteen self-named bindings in place to the D-5 `in <formal>_in = <formal>` form, which is the census obligation `B37-15` (`.project/active/elaborator-cutover/cutover-census.md`, `FIX-01`), and recovered the elaborator's enumeration-value reading. Both routes now produce a graph. At the Item 6 observation this row read `error: 15× SI_SELF_BINDING` / `—` / expected-collapse, basis "the contract requires the customer self-bindings to block" — true of the pre-migration model, which no longer exists. **Shipped input-key cells, measured at the recovery Slice 3E authority switch (2026-08-11):** legacy publishes **31** entry-point keys, the exact route **27**, sharing 13. Every delta is one mechanism — legacy names an entry point after the *consuming calc-usage formal*, the exact route after the *modelled attribute* that supplies it. Three modelled values collapse from several per-consumer keys to one (`gain`, `thermal_efficiency`, `availability`); ten rename one-to-one with a single consumer on both routes; one is the constraint threshold `hif_plant_pkg__hif_plant__viability__81ddf10fb1d1749b__threshold` → `…__viability__threshold`, which is **not** a collapse — one consumer on each side, and the disambiguating id is retained identically in the module id and evaluation channel, dropped only from the input key. Ten more keys keep their spelling and change group. Full enumeration in `.project/active/cutover-recovery/evidence/3e-package-comparison.md`; consumer sets are identical on both routes for every row. **[AGENT] (orchestrator ruling, 2026-08-11)** expected-fix; the disambiguator drop is flagged to the Phase 5 owner packet, because two same-named constraint usages under one owner would mint the same key. |
...
| 36 | `unresolvable_attr_probe` | graph 1/1/0/0 | graph 10/9/0/0 | M/E/O | expected-fix | Nine concrete modeled formulas are runtime behavior that legacy dropped. **Shipped-package cells, measured at the recovery Slice 3E authority switch (2026-08-11):** before, the package carried `inputs/system_design.json` with one literal attributed to the legacy deriver's synthetic `hierarchy` source; after, the exact route reaches generation with all ten modules and **generation refuses** with the fail-loud module-class collision `'DerivedComponent_inherited_productModule'` / `'DerivedComponent_mixed_productModule'`, because two of the nine newly resolved formulas alias to one class name. The two routes share no entry point, so no naming rule reconciles them. **[AGENT] (orchestrator ruling, 2026-08-11)** classified expected-fix at the switch; the collision is the generator's own guard working on modules legacy never produced, and the fixture is a graph-level probe that no test generates a package from. Fixture-internal; both halves flagged to the Phase 5 owner packet. **Two measured details added after the Slice 3E audit (F5), because the switch made this path reachable from public `generate`:** the guard raises a bare `ValueError` (`generation/registry.py:146`), so the operator sees **"Unexpected error"** with a traceback rather than the typed refusal this slice established elsewhere; and it fires inside `_generate_modules`, *after* the output tree was cleared, leaving a **half-written package** (measured: 34 files, no contracts, no seal). Neither is a regression this slice introduced — the guard and its handling predate it — but both are now product behaviour and both go to the Phase 5 owner packet. |
