...
- **D4. Each derivation carries a two-line `//` block; the full basis statement is written once at
  the `catf_radial_build` level.** *Rejected: repeating a full paragraph 27 times* — heavy and
  unreadable. *Rejected: a single file-level statement with no per-derivation text* — the prover
...
6. Every free radial parameter and every A9 operand carries unit text that **all** its consumer
   lanes agree on. This is now a build requirement, not a documentation preference.
...
| 6 | `tests/fixtures/catf_mfe_gated/PROVENANCE.md` | see below | the ruled table + Item 8's fix commit |
| 7 | `tests/fixtures/catf_mfe_gated/instance_graph_snapshot.json` | re-captured under license | the edited sources |
| 8 | `.project/backlog/BACKLOG.md` | one-line amendment (N-4 side 2) | the owner's SC-3 ruling |
...
measured-shape paragraph; §1's *"`ProductWithinBand` is deliberately not authored"*; §1's *"A5, A6,
A9 are left exactly as `catf_mfe_d5` wrote them"*; §1's *"the axis-region layer is not derived"*;
§2's heading and identity (`65 = 58 + 7` → `65 = 56 + 9`); §3a's parked-row block, retired with a
citation to `62a07e5`; §5's now-false claim that a constraint formal cannot carry unit text; the
Authority block's `SC-3's identity`. Add deletion records **D8 (A5)** and **D9 (A6)** carrying the
relation, the chosen basis, and the authorizing row. Record that A9 re-enters the SC-5 candidate set
as a record, not a re-disposition.

**Before/after public key set, to be recorded:** 26 keys leave (the derived radii that were entry
points; `axis_region.outer_radius` was never one), 16 arrive (14 radial free parameters +
`pump_capacity_each` + `pumping_speed_agrees__rel_tol`). Correct the spec's phrasing that the free
parameters "stay keys" — 14 of 15 were not keys before and *arrive*. Direction is as ruled.
...
- **Float drift — surfaced, not absorbed.** The spec's `[HARD]` claim that the derivations reproduce
  ...
  those five layers. **Recorded for the owner under capture-fidelity law 4.** Mitigation: do not
  re-baseline anything on it; if an execution expectation moves, that is the surfacing event the
  spec describes, not a number to absorb.
...
- **D3 is an edit outside the 27.** It is required for A6 to build and is flagged rather than
  buried. If the reviewer or owner reads it as unauthorised, the alternative is to surface the
  refusal instead — but the ruled form then cannot land.
...
2. `tests/conformance/test_gated_manifest_identity.py` green, including new per-occurrence
   falsification cases: strip one layer's comment block → exactly one problem naming that layer;
   remove one layer's initializer → exactly one problem naming that layer.
...
5. The generated package builds and seals; the recorded key movement is exactly the 26/16 sets above.

## Next-Stage Handoff

**Fixed** — the 27 derivations' shape and order (D4, D6, D7); the unit spelling (D1, D2, D3); the
prover's owning-block anchoring (D5); the expectation table above; the sequence
(source → read line numbers → commit expectations → run).
