...
The mechanical rule: `is_computed_attribute` branch → `FORMULA` arm; `is_aggregation` branch →
`AGGREGATION` arm; `else` → `CALCULATION` arm (unchanged body); a `constraint |
report_aggregator` arm raises via the shared constructor. Per-seam before/after:

- **Seam 1 — `_get_python_path` / `_raw_source_name` (`cli/__init__.py:150-174`).** The three
  `if/elif/else` arms keep their exact QN-derivation bodies, rekeyed to `module_kind`. `CONSTRAINT`
  / `REPORT_AGGREGATOR` raise. `_check_duplicate_output_paths` needs no logic change — it calls
  the two helpers, which now raise for unrenderable kinds (so a constraint module in the module
  list fails the pre-clear check loudly, which is correct).
...
- **Seam 4 — `_get_module_sysml_qn` (`stencils.py:34-47`) + auto-impl counting
  ...
  silently skipped).
- **Reader — pipeline source label (`pipeline.py:127-134`).** The ternary rekeys to
  `module_kind`. Item 6 keeps the two existing labels byte-identical; new kinds raise.
- **Reader — test-gen skip (`test_gen.py:47`).** `if is_computed_attribute or is_aggregation:
  continue` → `if module_kind in (FORMULA, AGGREGATION): continue`. New kinds raise (they must not
  silently fall through into the calc-conformance emitter).
...
ModuleKind.AGGREGATION`; `test_data_models.py:584-585` field inventory swaps the two flag names for
`module_kind` **and** adds `output_schema_type`. The two comparison harnesses
...
- **INV-3 (no silent fall-through).** No seam or reader routes a `CONSTRAINT` /
  `REPORT_AGGREGATOR` module to a calc path or skips it. Each raises. (Unit-tested here; exercised
  for real in Item 7.)
...
Per module object in each of the 9 `tests/fixtures/baseline_outputs/*/computation_graph.json`:
...
- **`unrenderable_module_kind_error(module, seam_name) -> CodeGenerationError`** (new, in
  `generation/` — colocated with `CodeGenerationError`) — builds the uniform fail-loud exception
  carrying module name, kind value, and seam name. Called from every seam's unrenderable arm.
...
- **INV-3 unit tests (new):** construct a `PipelineModule(module_kind=CONSTRAINT, …)` and assert
  each of the six sites raises `CodeGenerationError` with the kind and seam in the message. These
  are the tests that lock the fail-loud contract Item 7 exercises for real.
