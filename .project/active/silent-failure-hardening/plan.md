...
### Phase 4 Completion (Family 4 — Exception Swallows + SC-5)
**Completed:** 2026-07-06
**Gates:** suite 2018 passed; ruff 17; mypy 104. Clean sweep GREEN.
**Per-finding:**
- **D3-6** (`loader.py`): the `usage_type_map` malformed-key `except (JSONDecodeError, IndexError): pass` now LOGS the drop (WARNING) — the except was already narrow; a bare pass hid the offline-only mis-wire. Test deferred (needs a full hierarchy-dict scaffold); simple log-add, code-reviewed.
- **D3-12** (`parameter_groups.py`): the OperatorExpression default-eval `except Exception: return None` narrowed to `(ValueError, TypeError, AttributeError, ArithmeticError, KeyError)` + debug-log — a truly unexpected error now propagates loudly instead of silently returning None.
- **D3-14** (`preservation.py`): removed the broad `except Exception: return None` in `_extract_signature_from_impl`; the preserve-on-transient POLICY now lives at the call site (`should_regenerate_stencil`) — a non-empty impl that fails to read/parse is PRESERVED (warned), only a genuinely-empty impl regenerates (INV-4). **Behavior change:** the existing `test_req_sr_03_decision_tree_case2_unparseable` pinned the old buggy behavior (non-empty unparseable → regenerate/stub-over); flipped to assert preserve. Added `test_req_sr_03_empty_impl_regenerates` for the empty-file boundary.
- **SC-5** (`parameter_groups.py`): the `float()` root is ALREADY narrow (catches `(ValueError, TypeError)`). **The hazard-scoped WARN is DEFERRED.** A naive present-but-unparseable predicate over-fires on legitimate chain/reference design defaults (verified live: `attr_expr_probe`'s `half_vol = split.half` / `quarter_vol = split.quarter` are resolved elsewhere, not EP-drops) — it broke INV-6 on a clean fixture. Correctly scoping the warn needs the EP-omission membership check across the full derivation (fire only when the attribute feeds an entry point that is genuinely omitted from every group). That requires iteration against `plant_value_shapes` I could not land safely in this stage's budget without shipping a false-positive-prone heuristic. Deferred with this note; the eval/float roots are narrowed regardless. **Consequence:** `plant_value_shapes` does NOT become a trip fixture in this stage (no warn fires) — no re-capture, no exclusion-list change; INV-6 holds unchanged.
...
**Deferred set — COMPLETED in the follow-on session (2026-07-06, fresh budget):**
- **SC-5/D3-12 hazard WARN** — LANDED at JSON-emission time (`derive_groups` → `_warn_nonfloat_entry_points`): a None-default group param whose input type is non-numeric (bool/string/enum) is warned; numeric-typed and chain-reference defaults stay silent. Verified: fires only on `plant_value_shapes`' `wall : 'Wall Kind'` enum EP, silent on `attr_expr_probe` and the whole clean corpus (INV-6). `plant_value_shapes` is now the third expected-warning trip fixture (generated bytes unchanged — the fix warns, does not pre-fill). Tests in `test_silent_failure_sc5.py`. Commit `25f3a7a`.
- **SC-4 A1** — LANDED: EP-key uniqueness fail-fast in `extract_design_attributes` (where raw names still exist). New `ep_key_collision_probe` fixture + test. Commit `e62c67f`.
- **D3-16 synthetic fixture** — LANDED: `d316_crosspart_expose` (package-level calc usage exposed single-hop by a part) trips the else-warn; fires-on-shape test. Commit `891620a`.
- **Pattern-3 sentinels** — scoped-alias registration now has a scanned/registered zero-found sentinel + WARN-on-gap; design-override rewrite and self-named rescue already carried count-summaries. Commit under pattern-3.
- **Phase 6** — register §D3 Disposition column fully discharged (per-row fixed/reclassified/closed-by-construction); D3-7 row reclassified; `[D3-HYGIENE-TAIL]` + `[MULTIHOP-CHAIN-PARSE]` + Item-9 impacts filed in BACKLOG.md; the doc-19 dispatch-invariant doc updated with INV-1..5 + the code-review-checklist note. **Remaining doc-sweep** (per-component reference docs 01/10/12/13/14/16/17/23/27 + matrix rows) is a scoped follow-on — the per-finding Implementation Notes here are authoritative.

**Audit cures (PASS-WITH-NOTES → resolved, 2026-07-06):**
1. **D3-5 / D3-6 / D3-15 fires-on-shape tests** — added `tests/unit/test_silent_failure_family2_family3_fires.py`: D3-5 (registry Phase-1a unknown calc def warns, constructed usage), D3-6 (malformed `usage_type_map` key drop logged, constructed snapshot dict), D3-15 (two distinct design prefixes warn, constructed hierarchy + usages). Each expectation independently anchored to constructed inputs (R1).
2. **D3-12 numeric leg — DECISION: DEBUG is correct** (recorded at `parameter_groups._parse_default_value` + here). A numeric-typed attribute with an unparseable *expression* default (`Real = 1.0 / q_eng`, `half_vol = split.half`) is a SUPPORTED deferred shape resolved through the computed-attribute / expression-aware path — it does NOT land as a silently-omitted float EP (verified: such params never reach a None-default group). The loud SC-5 hazard is hazard-scoped at emission to NON-numeric types, which can never be float EPs and have no other resolution path. Promoting the numeric leg to WARN would over-fire on every legitimate expression default and break INV-6; the disposition of unresolved numeric expression defaults is owned by the expression-aware-codegen epic, not this item.
3. **Register discharged to 16/16** — D3-9 (→ tripwire), D3-13 (→ sentinel), the pattern-3 line (→ scoped-alias sentinel landed; remaining single-INFO sites low-value follow-on), and the hygiene-tail line (→ FILED `[D3-HYGIENE-TAIL]`) all moved off "Item 5".

**Final gate (follow-on close):** suite **2031 passed** / 4 skipped / 5 xfailed; ruff **17** (≤ baseline 19); mypy **104** (= baseline); clean-corpus zero-WARNING sweep GREEN; the three expected-warning trip fixtures (`deep_cross_scope_probe`, `plant_value_shapes`, and the D3-8/10/16/ep-key/d316 fixtures) pin their diagnostics.
