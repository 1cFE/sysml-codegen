...
| SR-A02 | SR-R23 | The same usage-owned attribute consumed by both a constraint actual and a calculation input yields **one** QN-keyed typed entry point, not two, with one modeled default and one group assignment. |
