...
- [x] **4. Replacement and matrix tests plus the `gain = 100` three-route proof.** **[AGENT]
      (ratified for execution by owner, 2026-08-12; executed 2026-08-14 per the rev-2 brief —
      the REQ-CL-03 pre-amendment check was superseded by the landed contract and replaced by
...
- [x] **6. Ruff R12 amendment.** **[AGENT] (ratified for execution by owner, 2026-08-12;
      ...
      no worse.
- [x] **7. Three complete final batteries at the final paired OIDs.** **[AGENT] (ratified for
      execution by owner, 2026-08-12; executed 2026-08-14 after the owner ruled the tree final —
      51/51 fields identical across three runs at `2819501`/`6372ef7`; the battery's first pass
      caught and cured the L-179 stale citation; completion record below.)** Run all three
      complete batteries after the substantive corrections, against one final codegen/agentic
      OID pair.
- [x] **8. Candidate-record regeneration.** **[AGENT] (ratified for execution by owner,
      2026-08-12; executed 2026-08-14 — `evidence/candidate.{md,json}` and
      `final-candidate-tables.md` regenerated by `build_candidate_final.py`, every number
      derived; completion record below.)** Regenerate one internally consistent candidate record
      from the final evidence and paired OIDs.
- [x] **9. Fresh narrow audit.** **[AGENT] (ratified for execution by owner, 2026-08-12;
      executed 2026-08-14 by an independent audit agent — `evidence/audit-9-final.md`, verdict
      **CERTIFY-WITH-RESIDUALS**, 0 blocking / 1 major / 3 minor, all record-accuracy or
      spec-text defects; completion record below.)** Audit
...
by the auditor, implementation notes treated as hypotheses). Report:
`evidence/audit-9-final.md`. **Verdict: CERTIFY-WITH-RESIDUALS — 0 blocking / 1 major / 3
minor.** All six charged subjects verify on their substance under the auditor's own probes:
symbol absence + 32 pinning tests (subject 1), R8 witness with hand-derived arithmetic
(subject 3), byte-identity pins with no checkout-absolute provenance (subject 4), every R12
gate value and both enumerated ruff sets reproducing live in both repos (subject 5), and the
record's 51/51 comparison, OIDs, hashes (79/79), quoted tables, and derivations (subject 6).

**The subject-5 ruling:** the R12 mypy clause's number is wrong, the gate is not failing. The
auditor extracted both Item-6 base OIDs read-only and measured them under the mandated venv:
agentic base is **118-in-28** (not the recorded 105-in-23) and the current 108-in-26 set is a
strict subset — zero new, ten fixed, reproducing plan.md:1850; codegen drifts identically
(71-in-17 recorded, 73-in-18 measured), pinning the cause to the pre-cutover `uv run`
measurement environment. Amending the literal numbers to the measured values is an owner
decision, exactly as disposition 2 handled the stale ruff 14.

**The findings, all record-accuracy or spec-text, none touching tree correctness:**

- **F1 (major):** the replacements sweep is **302 of 304 rows** — the checker silently skips
  non-codegen rows, and the two skipped agentic rows (L-036/L-037) cite `tests/unit/` paths
  that do not exist in the companion (the real files are `tests/test_sysml/…`, which the
  auditor ran: 65 passed — substantive coverage intact). Same stale-citation class as L-179,
  in the only rows the checker structurally cannot reach. candidate.md's "full 304-row sweep"
  is an overclaim by two rows.
- **F2 (minor):** R12's mypy baselines don't reproduce under R12's own mandated environment
  (ruling above). **F3 (minor):** T4 shows full-tree ruff at a scope R12 doesn't govern (the
  governed `src tests` arm passes with margin: 143/358 codegen, 121/127 agentic). **F4
  (minor):** T1's name-status line omits 18 renames.

**Nothing was fixed at step 9, deliberately.** F1's checker-disclosure half touches
`scripts/` — a shipped path — so fixing it invalidates the spent batteries; the
citation/wording halves are record repairs. Pricing that (accept with residuals; record-only
repairs; or full fix + battery re-run) is the owner's step-10 decision, not the audit's or
the orchestrator's.

#### Narrow-correction steps 7 and 8 completion

**Completed:** 2026-08-14, in one session directly after step 6, on the owner's tree-final
ruling (**[OWNER 2026-08-14]** "no changes in flight", ruled at `540ad59` after the
`/_my_ask_me` reframing recorded in session; steps 7–9 explicitly directed).

**Step 7 — the batteries.** Machinery derived path-only from the proven REVISE step-7a
scripts (derivation diff verified in session): `run_final_battery.sh` + `chain_final.sh` +
`compare_final_runs.py`, committed at `2819501`. Three consecutive complete runs into
`evidence/phase5-runs/final-runs/`, each opening with the hard environment gate (imports
pinned to the two `item7-rebuild` worktrees + pinned TEAx; license key present) and recording
its HEAD OIDs. **Result: 51/51 compared fields identical across all three runs**
(`final-runs/comparison.md`, comparator exit 0), at codegen `2819501` / agentic `6372ef7` /
TEAx `75eecb3`. Headlines, identical in all three: codegen suite **2086/34/88 with zero
license-skip lines** (`-rs` proof); agentic suite **1831/1/5**; execution lane **88** on the
canonical scope, including the gain=100 three-route mutation proof (register row-3 single-shot
...
L-179 still cited `test_the_two_packages_differ_only_in_provenance_comments`, which step 5
renamed to `test_the_two_packages_are_byte_identical` (step 5 re-pointed L-007/L-010 and
missed this third citation; the full replacements sweep had not run since REVISE). The chain
was stopped, the citation re-pointed via `scripts/_ledger_edit.py` (row green 10+1; full-ledger
static sweep found no other stale citation), the repair committed (`2819501`, shipped-path
diff vs the ruled `540ad59` **empty**), the partial pre-repair runs discarded, and all three
recorded runs measure the repaired tree. One consistent state, no mixed evidence.

**Step 8 — the record.** `evidence/candidate.{md,json}` and
`phase5-runs/final-candidate-tables.md` regenerated by `build_candidate_final.py`: run values
parsed from the committed logs through the comparator's own parser, repository facts from
Git, hashes from bytes — nothing hand-typed; the shipped-diff-vs-ruled-OID emptiness is
builder-asserted inside the record. The record states its supersession (gate-2 record retired
as acceptance candidate, kept as evidence), quotes the tables verbatim, and surfaces four
things instead of resolving them silently (candidate.md §6): the L-179 catch-and-cure; **the
R12 agentic-mypy clause discrepancy** (spec says ≤105-in-23; every recorded measurement this
recovery produced says 108-in-26, endpoint of a recorded 118→108 improvement — parked to the
step-9 audit and the owner, since disposition 2 ratified amending the ruff clause only); the
TEAx docs-only pin move; and the known pre-existing broad-scope lane artifact and
stale-baseline classes.

**Steps 7–8 are spent.** Any further substantive change to shipped paths invalidates the
batteries and the record; the next steps are 9 (fresh narrow audit) and 10 (owner).
...
**Completed:** 2026-08-14, executed in-session by owner direction (option 2 of the recorded
execution-mode decision). Every row disposition below was probed, not trusted; every cited test
was run before its citation was written.

**Row dispositions.**

- Nine legacy UNTESTED rows → PASS on behavioral replacements through the public route:
  REQ-GEN-03 + REQ-OSR-02/03/05 via `tests/conformance/test_output_schema_contract.py`
  (`run_codegen` from the committed fusion_tea v6 snapshot; schema choice, named `MultiOutput`
  subclass, exact ordered field names, pydantic-required fields); REQ-SR-01/02/06/07 via
  `tests/conformance/test_smart_regen_behavior.py` (preserve / type-change / field-change /
  order / blanket-skip / all-kinds arms, license-free); REQ-GA-05 via
  `tests/unit/test_data_models.py::test_computation_graph_field_set_is_the_reviewed_boundary`
  (exact six-field set plus the serialized/in-memory `exclude` split).
- REQ-DIAG-04 → PASS: `tests/conformance/test_envelope_severity_tripwire.py` pins that across
  every committed v6 snapshot the only `severity`-keyed JSON path is the disposition severity;
  REQ-DIAG-01 stays PARTIAL as a recorded deliberate grade.
- REQ-EPC-01 and REQ-GA-03 → PASS with their missing failing arms:
  `tests/conformance/test_projection_wiring_contract.py` (closed three-value classification,
  membership totality over all committed snapshots, an authored 27-key fusion_tea oracle covering
  all three classes; absent-producer refusal `SI_EDGE_DANGLING` at both `validate()` and
  `project()`).
- REQ-CL-03 → amended per the owner ruling (usage domain is the totality boundary); the
  divergence note is resolved into a decision record; live citations
  (`test_a_usage_only_catalog_fingerprints_deterministically` et al.) run green.
- REQ-CS family minted (8 rows, Items 3/8/9; Item 5 mapped to REQ-CL-04/REQ-EXT-09 by recorded
  decision; TEAx-side pins named as cross-repo evidence). SC boxes ticked in the epic archive and
  item archive; `[CONSTRAINT-GATES-UNTAGGED]` marked complete; BACKLOG baselines updated.

**One product defect surfaced and cured (stop-and-surface honored, owner ruled in-session).**
Smart regen's exact-set field comparison made the generator's own output fail its own
preservation check on definitions with declared-but-unused inputs (`PVModuleCostCalc`,
`RackingCostCalc` in `solar_battery_d5`) — perpetual churn, handwritten edits stubbed over on
every regen. Cure **[AGENT] (ratified by owner, 2026-08-14)**: subset comparison in
`generation/preservation.py` `matches()` (referenced ⊆ declared preserves; an out-of-set
reference still regenerates). Doc 23 and the REQ-SR rows amended; no generated bytes changed.

**gain = 100 three-route proof.** `hif_plant_pkg__hif_plant__gain` feeds exactly
`lcoe_calc.gain_in`, `recirc_calc.gain_in`, and the viability constraint's `gain_in` (the
negative set enumerated over all 40 ports); its modeled default (80.0) is discovered from the
graph. At exactly 100 on live, in-place-snapshot, and relocated-snapshot routes the movers are
exactly `lcoe` and `f_recirc` (hand-arithmetic values), and the constraint demonstrably consumed
the injection: observed `gain_in` 100.0, margin 25.0 (= 0.35·100 − 10), verdict satisfied,
report headline `full_satisfaction`. A second leg at gain 20 flips the verdict
(`violated`/`violation`, margin −3.0) with the mover set gaining exactly the constraint response
and headline. Six-state vocabulary throughout. 29 passed (20 existing + 9 new) across all three
routes. Per register row 3, the authoritative single-shot observation re-runs at the steps 7–8
final paired OIDs.

**Matrix recount (mechanical, index + per-family + A-4 file method):**
**288 rows / 156 PASS / 1 PARTIAL / 131 RETIRED / 0 UNTESTED / 0 DEFERRED / 34 families /
64 kept test files, zero cited files missing from disk.** Delta from 280/136/3/131/10/0/33/55:
+8 CS rows; ten UNTESTED and two PARTIAL closures; nine newly cited kept files. Summary block,
"Untested Requirements" section, and both BACKLOG baseline lines updated; spec SC9 unticked at
start and reticked at completion.

**Gates.** Full licensed suite **2085 passed / 34 skipped / 88 deselected**, **zero**
`no live syside license` lines; node delta explained (+15 non-execution tests selected, +9
execution-marked gain arms deselected by default; collect `2119/2207 (88 deselected)`).
Execution lane **88 passed** (65 at step 3 + 14 epic coverage tests + 9 gain arms).
`capture_v6_batch.py --verify` **15/22/0** (unchanged); corpus selection **9 passed**. Ledger
`paths` **304/0**, `surface` **0**, all six `groups` READY; replacements sweep exit 0; proof
integrity **0/0**; doc distinctness **31/0**; retirement worklist check **0 problems**; gated
manifest identity closes **65 = 56 + 9**. `ruff check src` **12** (measured at preflight, was
14 at step 3 — decreased during the epic; zero-new held) and changed files clean; `mypy src`
**52 errors in 11 files** (was 57 — decreased during the epic; zero-new held).
`git diff --check` clean.

**Declared path set (exact).** Tests: the four new conformance files above,
`tests/unit/test_data_models.py`, `tests/execution/test_fusion_tea_mutation_teax.py`.
Production: `src/sysml_codegen/generation/preservation.py` only (the probe-exposed defect, owner
ruled). Docs: `docs/architecture/verification-matrix.md`,
`docs/architecture/reference/23-smart-regen-preservation.md`. Records: this plan,
`.project/active/elaborator-cutover/spec.md`, `.project/CURRENT_WORK.md`,
`.project/backlog/BACKLOG.md`, and dated amendments in
`.project/completed/20260814_epic_constraint_semantics_contract.md` and
`.project/completed/20260814_constraint-docs-agent-sync/spec.md`. No fixture, snapshot, schema,
or generated-baseline byte changed.
