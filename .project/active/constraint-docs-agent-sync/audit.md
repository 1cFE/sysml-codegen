...
**Commit audited:** `5e2373f`, plus three audit cures written on top (C-1..C-3, below)
...
Three defects survived my probes and are cured here (C-1..C-3); three findings stand (A-1..A-3), one
of which is a documentation defect the item itself introduced and my probe caught. The two residuals
the orchestrator ruled on are both loudly recorded; the SC5 residual was missing its named vehicle,
which is now filed.
...
**Product-lens ledger gate: DISPOSED.** Full block appended to `product-lens.md` (audit run). No
block in the ledger's history is unresolved: spec-stage DISPOSED (item7-F1..F3, all three folded into
the spec body), implement-stage CLEAR. Epic gate CLEAR. No owner-graded or `[HARD]` contradiction.
...
**The finding that does control something.** A-3 below is a real product contradiction: the
`@inapplicable:` example the new authoring doc tells a modeler to copy is refused by the shipped
generator. It is agent-grade, in a companion repo, named with its reproduction and with a verified
working alternative — so it is a residual, not a Needs Work. But it is the exact failure class this
item exists to prevent, one level up, and it should be fixed before the epic closes.
...
**CERTIFY-WITH-RESIDUALS.** Four of six epic success criteria are met and verified by re-running the
...
3. **A-3, the refused `@inapplicable:` example** — an agent-facing documentation defect across three
   surfaces. Reproduced with a verified working alternative. Should be fixed before epic close.
4. **A-4, the unreproducible test-file count** — cosmetic; record the counting method.
