...
## Amendments — recorded after implementation, pending owner ratification

Both entries are `[AGENT amendment, re-derived 2026-08-12 — pending owner ratification at the
gate]`. They record where the shipped implementation deviates from a decision above, why the
deviation is the better answer against that decision's own recorded reasoning, and what pins the
shipped behaviour. Neither was rewritten silently: the decisions above stand as written, and
these amend them.

### A1 — D3's one source-admission owner does not cover the live route

**The decision, as written.** D3 says live and capture both call one
`load_and_elaborate(admission, design_path_filter) -> InstanceGraph` after a private staged
admission has fixed the document set and bytes, so freshness can compare against the same
manifest.

**What shipped.** Capture does exactly that: `admit_sources` stages the documents, and
`elaborate_admitted_sources` elaborates the staged set (`snapshot/capture.py`,
`orchestration/elaborated_pipeline.py:116`). The live route does not. `build_elaborated_pipeline`
reads the caller's paths directly (`orchestration/elaborated_pipeline.py:59`), so a live node's
`source_file` is a checkout-absolute path where a captured node's is the staged referent
`root-N/...`. A conformance node pins the split deliberately —
`tests/conformance/test_snapshot_v6_routes.py::test_the_live_arm_does_not_share_the_capture_route`
asserts that `build_elaborated_pipeline`'s body names neither `admit_sources` nor
`elaborate_admitted_sources`.

**Re-derived against D3's own reasoning.** D3's stated purpose is that the claimed bytes cannot
race the parsed semantics — hash what SysIDE actually parsed, not what the filesystem held
afterwards. That purpose is fully served by capture alone, because capture is the only route
that *writes a claim about bytes*. The live route makes no manifest and no staleness judgment,
so there is nothing for it to race. D3's rejected alternatives were about hashing after parse
and about promoting `build_elaborated_pipeline` to a second public authority; neither is what
the split does, and there is one public authority either way (`run_codegen`).

The pinned split also buys something D3 did not weigh: it keeps the two arms of the route-parity
comparison independent. If live went through admission, a defect inside
`elaborate_admitted_sources` would move both arms at once and the equality assertion in that same
test would be blind to it. That is the reason the pin exists, stated in its own docstring.

**Why it is not converged here.** Routing live through admission would change every live node's
`source_file` from a checkout-absolute path to a staged referent. That is precisely
**audit-F4** — route-dependent generated bytes and portable live provenance — which is an **open
owner question** (`owner-disposition-20260811.md`, question 5). Converging D3 would decide F4 by
side effect, in the direction of "make live provenance portable", with no authority to do so. The
split is therefore left as shipped and recorded here, and the pin is left un-inverted.

**Amendment.** D3 is narrowed to: *staged source admission is the capture route's owner, and the
only route that claims bytes on disk. The live route elaborates caller paths directly, and the
separation is pinned.* Whether the routes converge is bound to audit-F4 and belongs with that
answer.

**audit-F4 answered, 2026-08-14 (Item 7 correction step 5, per disposition 4).** The routes do
not converge; the *rendering* does. The live route keeps elaborating caller paths directly and
now rewrites every node's `source_file` through `map_live_source_referent` — the same mechanism
it already used for `exclusion_location` — so both routes render one portable `root-N/` referent
shape, each from its own evidence. The route-parity test's recorded premise that making live
provenance portable "means routing it through admission" turned out to be false; the
arm-independence pin stands un-inverted, and generated bytes are equal across routes
(invariant 35 as amended).

### A2 — R2's envelope omits the designed `capture` object

**The decision, as written.** The envelope field table fixes a `capture` object with
`model_name`, `captured_at`, `producer`, and `options`, integrity-bound. Spec R2 requires the
envelope to carry "model identity, capture provenance, a portable source-staleness manifest, the
validated instance graph and its schema/fingerprint, and every version/certifiability marker
needed to reject semantic skew."

**What shipped.** There is no `capture` object. The outer keys are `format`, `version`,
`authority`, `sources`, `instance_graph`, `integrity` (`snapshot/envelope.py`). The producer
versions R2 wanted are carried inside `authority` (`sysml_codegen_version`,
`agentic_mbse_version`) alongside the compatibility markers. `model_name`, `captured_at`, and
`options` are absent, and the module docstring states the narrower guarantee in full.

**Re-derived against R2's own reasoning.** R2's load-bearing rule is that every field able to
affect identity, staleness, validation, projection, output, or certifiability must be
integrity-bound **and validated**, *or* explicitly classified as non-authoritative and proven
unable to affect those outcomes. Each omitted field fails the first half and could not honestly
pass the second:

- `model_name` is unverifiable even with the sources in hand. Integrity-binding it proves only
  that nobody edited it after sealing, and the digest is unkeyed, so a re-sealer can set it to
  anything. It would read as identity while proving none. `sources` is the field that can be
  checked — passing `source_roots` recomputes the admission from the model tree and must
  reproduce the sealed manifest exactly.
- `captured_at` is unverifiable in the same way, and actively harmful: it would make capture
  non-deterministic, and byte-determinism is what the accepted recapture batch's `--verify` gate
  rests on (15 captured / 22 refused / 0 deviations, re-derived on demand).
- `options.design_path_filter` has no value left to carry. Gate 4B-G0 removed the flag, so
  argparse rejects it before a snapshot is opened.

So the shipped envelope satisfies R2's rule *better* than the designed one: it carries what it
can anchor and drops what it could only assert. The design's envelope is not needed, and
building it would add three fields that look like provenance and prove nothing.

**Amendment.** The R2 envelope is narrowed to: *anchored constants, anchored environment facts,
the anchored graph, and a self-declared `sources` manifest that becomes provenance when the
caller passes `source_roots`. No unverifiable identity field is carried.* The limit is stated in
`snapshot/envelope.py`'s module docstring and pinned by the v6 envelope conformance nodes.
