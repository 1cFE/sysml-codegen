...
**Status:** Draft — design-review findings incorporated; D10/D11 remain open
**Owner:** Reid W
...
- **Task-local corpus verifier**
  (`.project/active/qualified-reference-occurrence-anchoring/`): produces the before/after typed-edge
...
Two evidence questions remain open: B3/D10, the authorability of a discriminating bare reference,
and D11, an affected deep-override shape. The plan must not begin production edits until their
learning tests determine whether kept fixtures exist and the owner disposes any resulting standing
gap. The highest implementation risk after those gates is fixture construction, not the resolver
change. Establish the combined consumer fixture before production edits so syntax or SysIDE
behavior fails at fixture cost.

---

**Next Step:** Resolve D10/D11, complete the design self-review, then after approval proceed to
`/_my_plan`.
