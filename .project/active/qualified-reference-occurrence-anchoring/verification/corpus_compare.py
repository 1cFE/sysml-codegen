...
``.project/active/qualified-reference-occurrence-anchoring/``. Run it once before the
...
def diagnostic_rows(
    ...
    return sorted(
        ...
        if diagnostic.consumer == consumer_id
        and diagnostic.param_name in (None, param_name)
    )
...
def capture_root(root: str) -> dict[str, Any]:
    ...
    result["diagnostics"] = sorted(
        [diagnostic.code.value, diagnostic.consumer_display, diagnostic.param_name,
         diagnostic.detail]
        for diagnostic in graph.diagnostics
    )
...
def identity_records(graph: InstanceGraph) -> dict[str, Any]:
    ...
    return {
        ...
        "constraint_nodes": sorted(
            node.node_id.to_wire() for node in graph.constraints.values()
        ),
    }
    ...
    for name, captured in sections.items():
        ...
        counts[name] = {
            ...
            "deep_override_one_segment_sites": sum(
                item.get("deep_override_lane", {}).get("one_segment_sites", 0)
