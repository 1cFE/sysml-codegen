...
from spike_pkg.modules.s6qualsiblingscope.unitcost import UnitCostModule

from spike_pkg.schemas.s6_qual_sibling_scope_params import S6QualSiblingScopeParams as S6QualSiblingScopeParams
...
def create_spike_pkg_registry() -> PipelineModuleRegistry:
    ...
    return create_registry(
        [            UnitCostModule,        ],
        module_type_override={            UnitCostModule: "s6qualsiblingscope.UnitCostModule",        },
    )


# Custom schema types for TEAx pipeline registration
# Use with: execute_pipeline(..., custom_schema_types=CUSTOM_SCHEMA_TYPES)
CUSTOM_SCHEMA_TYPES = [    S6QualSiblingScopeParams,    Float,]
