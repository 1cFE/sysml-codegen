# Prompt: Interactive Pipeline Explainer (post-CONSTRAINT-EXEC)

Copy everything below the line into a fresh agent session in
`/home/reid/1cfe/sysml-codegen`, on branch `constraint-exec-epic` (or `main` once that
branch merges). Two epics landed since the Gen-1 explainer: **PIPELINE-TRUTH** (whole-plant
value resolution, the F4 aggregation cutover, silent-failure hardening) and then
**CONSTRAINT-EXEC** (modeled assertions now *execute* — they lower to constraint modules
evaluated under three-valued Kleene semantics, snapshots went to v3, and packages seal). The
sysml-codegen reference docs were re-projected onto this HEAD in the docs-explainer-refresh
sweep; trust them and cite them rather than re-deriving.
...
current as of `constraint-exec-epic` HEAD. The facts below carry through: the GREEN
...
generated package alone, and CONSTRAINT-EXEC then added the constraint-execution path on top
of that without disturbing the constraint-free byte-identity baselines.
...
- **L1 — The pipeline shape.** Extraction → analysis/resolution → ComputationGraph →
  generation, plus the snapshot path (`generate --from-snapshot`, now format v3) as a
...
| `snapshot/` (doc 27) | Versioned extraction capture (now **v3**: adds `constraint_facts`, `part_occurrences`, `constraint_lowering_mode` so the offline path re-lowers constraints); regeneration from it is byte-identical to live | Generation, debugging, and CI shouldn't require the live parser toolchain on every run |
...
| `contracts/` (doc 29) | Emit the package's semantic `ModelContract` (graph-only) + physical `PackageContract` seal + a verbatim verifier; the `seal` CLI re-seals in place | A consumer must confirm it loads exactly the bytes generated, and know what it may vary/observe without parsing YAML |
...
generation boundary. Extend the same frame to the two CONSTRAINT-EXEC surfaces:
...
real at post-CONSTRAINT-EXEC HEAD. State them exactly as hedged; do not present them as
solved, and do not resurrect the pre-epic caveats the two epics retired (see the note below).
...
- **The constraint catalog is embedded on the graph, not emitted standalone, and the bridge
  is single-channel.** Standalone `constraint_catalog.json` emission (CE-F1) and a
  multi-channel `CandidateBridge` (CE-F2) are filed follow-ons, not landed. Describe the
  current embedded-catalog / single-channel reality; reference CE-F1/CE-F2 as open, never as
  behavior.

**Retired by PIPELINE-TRUTH and CONSTRAINT-EXEC — do NOT present these as open caveats**
(they were true earlier; they are not now):
...
- **"`resolve_input()` / `AGG_STRATEGIES` is built but not yet wired"** — RETIRED. The F4
  cutover landed (TRUTH-DEBT Item 1): the live aggregation path now calls
  `resolve_input(AGG_STRATEGIES)` and the old `_resolve_aggregation_input_channel` was
  deleted. It is the live path, not a parity-only consolidation.
- **"Snapshots are format v1/v2"** — RETIRED. The format is v3 (constraint sections added);
  there is no coexistence — an old snapshot is a hard error and is re-captured at v3.
...
- **Lowering** ([P1 RESOLVE], `analysis/constraint_lowering.py`, doc 28): the executable
  ...
  owner-instance (the same resolution ladder calc inputs use); an admitted usage becomes
...
(default-on, its GRANDFATHERED carve-out now empty), **not** a live drop path; the
`collect_constraint_manifest` collector **survives** (a kept migration-mapping test needs
it — only the report/render/serialize surface was retired); and CE-F1 (standalone
`constraint_catalog.json` emission) and CE-F2 (multi-channel `CandidateBridge`) are **open
follow-ons**, not landed — the current reality is the embedded catalog and a single-channel
bridge.
...
## Reading list (in order; re-verified at CONSTRAINT-EXEC close / docs-explainer-refresh sweep)
...
4. Reference docs per the responsibility table above (01, 25, 12, 16, 10, 11, 24, 02, 07,
   17, 08, 20, 21, 27; 19 for the AST invariant; **09** for `ModuleKind` + `ConstraintCatalog`;
   **28** for constraint lowering / catalog / predicate compiler; **29** for contracts / sealing)
5. `docs/architecture/verification-matrix.md` — skim; the REQ counts and families
   (**274 = 273 PASS + 1 UNTESTED, 32 families** — the new `CON` family is contracts/sealing)
   if you cite coverage. Recount from the family index, never the summary block.
...
7. `.project/completed/20260713_epic_constraint_execution.md` — the CONSTRAINT-EXEC epic
   (constraint lowering, catalog, predicate compiler, contracts/sealing, snapshot v3) +
   `.project/research/20260706_pipeline-truth-discovery.md` — the PIPELINE-TRUTH discovery
   register (D1–D7 + adversarial); the evidence base for the earlier epic's mechanisms
8. `.project/backlog/epic_pipeline_truth.md` — the PIPELINE-TRUTH epic (goals, per-item
...
HTML now carries a deprecation banner — it predates **both** the UPSTREAM-FINDINGS /
PIPELINE-TRUTH **and** CONSTRAINT-EXEC epics, so treat its *content* as stale against HEAD.
The split is roughly: its **rendering machinery + 4-act frame are ~70–80% reusable** (viewBox
zoom/pan controller, tier-slot DAG layout, narrative callouts, progressive disclosure — lift
these), while its **content/data layer is largely a rewrite** (it knows nothing of the
supplied-value materializer, the F4 cutover, constraint lowering, `module_kind`, Kleene
predicates, the catalog, contracts/sealing, snapshot v3, or the teax study layer). Do not
