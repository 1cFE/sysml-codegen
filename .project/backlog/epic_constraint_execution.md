...
- [ ] `PackageContract` seals generated artifacts (content hashes, verified on load); `ModelContract` derives solely from graph fields; the study layer runs list/grid studies with atomic case commit, three-layer identity, proposal records for invalid candidates, and fingerprint-bound resume — the S6 invariants as kept tests. **Evidence:** Item 9 (certified: content-hash seal verified on load, tamper/extra/stale diagnostics, graph-only ModelContract, fingerprint stability both legs) + Items 10–12 (certified: S6 invariants as kept tests, fence mutation RED/GREEN, CLI crash-resume acceptance). Item 14's slice: `GENERATOR_MISMATCH` seam documented-and-removed (commit `78951a4`). **Known narrowing (follow-ons registered in BACKLOG):** the fusion-tea acceptance surfaced three integration gaps bridged consumer-side — no standalone `constraint_catalog.json` emission (Item 9 surface), single-channel `CandidateBridge` (Item 12 surface), `PreparedEvaluator` hardcoded fixture class (Item 10 surface).
