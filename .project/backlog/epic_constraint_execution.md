...
- [x] **Acceptance — met with recorded divergence** *([OWNER] ratified 2026-07-13: "99.7% + 7 model-favoring boundary rows" accepted as satisfying the Critical Success Factor, no `>`-matched re-run; the independent audit confirmed the 7 divergences are exactly the boundary set, uniformly model-favoring, epsilon window excused nothing — see `epic_constraint_execution_audit_independent.md`)*: the hand-coded rule is deleted and the sweep runs the generated `'Viability Threshold'` assertion via the study layer; **2294/2301 grid classifications match exactly (99.7%)**; the 7 divergent rows are exactly the `eta*gain == 10.0` boundary, where the hand rule's strict `>` was *unfaithful to the model's* `>=` — the epic's thesis (hand-rule drift caught and corrected) proven as data, never a bare "100%" (fusion-tea `exploration/ife_e2e/study/`, acceptance report + findings.md; ~168× prepare-once benchmark, 200/200 verdict parity). Originally: **Pending S-FUSION** (Item 14 Appendix C) — this repo's W1 gain fix is the precondition and has landed (`hif_plant_pkg__hif_plant`'s `'Viability Threshold'` now lowers live; commits `9dcd1ab`/`8d337bd`); the acceptance run itself is out of this repo's sandbox.
- [x] Every assertion in the executable profile lowers to a module or blocks generation naming identity, construct, and source location; non-assert kinds appear in the catalog as unassessed; the drop manifest and both blanket warnings retire with a 1:1 migration mapping. **Evidence:** Items 5-9 (prior); Item 14 closes the last extraction-side gap (gain fix, commits `9dcd1ab`/`8d337bd`) and proves + retires the manifest (`test_constraint_migration_mapping.py`, commits `cd7a204`/`b994112`) — `grep` confirms zero drop-manifest emission / blanket warnings in `src/`.
- [x] Live and snapshot generation produce byte-identical artifacts and identical constraint IDs/catalog ordering; a pre-constraint-facts snapshot or a current one missing the section is rejected with a re-capture instruction — never loaded as an empty catalog. **Evidence (Item 14 slice):** the from-snapshot rebuild path (`graph_rebuild.py`) mirrors the live gain-fix widening exactly, verified by the `plant_values`/`fusion_tea` re-capture + byte-identity gate (commit `8d337bd`) and by folding both into the standard `test_live_vs_snapshot_byte_identical` parametrization (commit `8d337bd`). The version-rejection guarantee itself is Item 8's, unchanged by Item 14.
- [x] A violated assertion completes with ordinary outputs intact and evidence (actual value, margin where structure fixes its sign, observed operands); violation, indeterminacy (Kleene, non-finite), and execution failure are three distinct queryable outcomes at module, report, and study level. **Evidence:** Item 7 (audit: violated-never-raises + ordinary outputs asserted under real simkit; SC-2 indeterminate + margin tests, Phase 6 cures), Item 10 (three-distinguishable-outcomes kept test), Items 11–12 (three case states × verdict classes queryable; certified audits).
- [x] The report aggregator has a generated exact input schema (missing result = schema failure), exists even for zero assertions, and is a *guaranteed* exit ancestor (explicit membership or generation-time assertion, not incidental). **Evidence:** Item 7 (certified audit: exact-schema aggregator, zero-assertion case, falsifying narrowed-exit test with a control leg, break-the-YAML executor-failure test).
- [x] `PackageContract` seals generated artifacts (content hashes, verified on load); `ModelContract` derives solely from graph fields; the study layer runs list/grid studies with atomic case commit, three-layer identity, proposal records for invalid candidates, and fingerprint-bound resume — the S6 invariants as kept tests. **Evidence:** Item 9 (certified: content-hash seal verified on load, tamper/extra/stale diagnostics, graph-only ModelContract, fingerprint stability both legs) + Items 10–12 (certified: S6 invariants as kept tests, fence mutation RED/GREEN, CLI crash-resume acceptance). Item 14's slice: `GENERATOR_MISMATCH` seam documented-and-removed (commit `78951a4`). *(Checked [OWNER] 2026-07-13: the criterion's own mechanisms are certified and were re-verified by the independent audit — Item 9's extra-sweep mutation probe reproduced verbatim.)* **Known narrowing (registered follow-ons):** the fusion-tea acceptance surfaced three consumer-side-bridged integration gaps; CE-F3 (hardcoded fixture class) is FIXED (teax `0d606a4`, 2026-07-13); CE-F1 (standalone catalog emission) and CE-F2 (multi-channel `CandidateBridge`) remain open in BACKLOG.
- [x] Byte-identity gates stay green everywhere the epic touches existing behavior: current fixture corpus regenerates byte-identically (timestamps excepted) until the deliberate calc-seam cutover, which lands under its own byte-identity gate. **Evidence (Item 14):** full 29-fixture re-capture + timestamp-only-diff gate after the W1 gain fix (exactly `plant_values`/`fusion_tea` changed, commit `8d337bd`) and again after the W2 retirement's `dropped_constraints` key removal (same two files, commit `b994112`). Item 13's calc-seam cutover is a separate, already-landed item.
- [x] All existing suites pass in all three repos; new behavior is covered by the concept's validation strategy (expansion, strict resolution, ID determinism, live/snapshot parity, report precedence, crash-resume). **Final sweep (orchestrator, 2026-07-13):** sysml-codegen 2330 passed / 23 skipped, mypy 76 baseline, ruff clean; agentic-mbse 1401 passed / 1 skipped; teax green except the 4 known pre-existing `test_no_battery_deps` failures (hardcoded-path bug predating the epic).
- [x] Documentation: authoring guidance teaches executable constraints and the profile's block list; architecture docs cover the new pipeline phase, catalog, contracts, and study layer. **Evidence:** sysml-codegen commit `ccfe9db`; agentic-mbse commit `d83109a` (constraints.md flipped to the profile); teax Appendix-B commits (evaluator + study docs, tracking-key note).
...
**Success Criteria**:
- [x] Migration mapping test green; `grep` finds no drop-manifest emission or blanket warning. **Evidence:** `tests/conformance/test_constraint_migration_mapping.py` (11 tests, sysml-codegen commits `cd7a204`/`b994112`); `grep -rn "collect_constraint_manifest\|render_constraint_report\|report_dropped_constraints\|not executable\|dropped_constraints" src/` returns zero except the kept `collect_constraint_manifest` sweep the mapping test itself calls.
- [ ] IFE grid classifications match 100%; the hand-coded rule is deleted from the sweep harness. **Pending S-FUSION** (fusion-tea session, Appendix C) — not startable until this repo's W1 gain fix (landed, commit `9dcd1ab`/`8d337bd`) and teax's W5b loader seal wiring (S-TEAX, parallel) both land.
- [~] Docs updated in all three repos; epic Success Criteria checklist fully checkable. **sysml-codegen: done** (commit `ccfe9db` — modeling-assumptions.md §8 flipped, `reference/28-constraint-lowering-and-catalog.md` added, verification-matrix.md CL family + recount). **agentic-mbse/teax: pending** S-MBSE/S-TEAX sessions (Appendices A/B, running in parallel per the orchestrator).
