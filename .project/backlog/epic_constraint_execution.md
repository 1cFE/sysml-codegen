...
- [ ] **Acceptance**: the IFE sweep's hand-coded viability rule is replaced by the generated assertion and every existing grid classification matches (concept Validation Strategy). **Pending S-FUSION** (Item 14 Appendix C) — this repo's W1 gain fix is the precondition and has landed (`hif_plant_pkg__hif_plant`'s `'Viability Threshold'` now lowers live; commits `9dcd1ab`/`8d337bd`); the acceptance run itself is out of this repo's sandbox.
...
- [ ] A violated assertion completes with ordinary outputs intact and evidence (actual value, margin where structure fixes its sign, observed operands); violation, indeterminacy (Kleene, non-finite), and execution failure are three distinct queryable outcomes at module, report, and study level. **Out of Item 14 scope** — Items 5-7's contract; not re-verified by this item.
- [ ] The report aggregator has a generated exact input schema (missing result = schema failure), exists even for zero assertions, and is a *guaranteed* exit ancestor (explicit membership or generation-time assertion, not incidental). **Out of Item 14 scope** — Item 7's contract.
- [ ] `PackageContract` seals generated artifacts (content hashes, verified on load); `ModelContract` derives solely from graph fields; the study layer runs list/grid studies with atomic case commit, three-layer identity, proposal records for invalid candidates, and fingerprint-bound resume — the S6 invariants as kept tests. **Item 14 touched only the reserved `GENERATOR_MISMATCH` seam** (document-and-remove, commit `78951a4`); the study layer itself is teax's (Items 10-12), out of this item's scope.
...
- [~] All existing suites pass in all three repos; new behavior is covered by the concept's validation strategy (expansion, strict resolution, ID determinism, live/snapshot parity, report precedence, crash-resume). **sysml-codegen: green** — 2330 passed / 23 skipped, `ruff check src/` clean, `mypy src/` 76 (baseline unchanged) as of commit `78951a4`. **agentic-mbse/teax: pending** S-MBSE/S-TEAX sessions.
- [~] Documentation: authoring guidance teaches executable constraints and the profile's block list; architecture docs cover the new pipeline phase, catalog, contracts, and study layer. **sysml-codegen: done** (commit `ccfe9db`). **agentic-mbse (profile authoring surface) / teax (evaluator + study layer): pending** S-MBSE/S-TEAX sessions.
