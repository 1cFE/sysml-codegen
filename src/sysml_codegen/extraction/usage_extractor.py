...
_SOURCE_BYTES_CACHE: dict[str, bytes] = {}
...
def _written_reference_text(expr: Any) -> str | None:
    ...
    which is the same adapter surface extraction already uses for source locations. This
    reads that span. Returns ``None`` when the span is unavailable rather than guessing.
    """
    ...
    except Exception:  # noqa: BLE001 - a missing location is an absence, not a failure
        return None
    if not source_file:
        return None
    key = str(source_file)
    data = _SOURCE_BYTES_CACHE.get(key)
    if data is None:
        try:
            data = Path(source_file).read_bytes()
        except OSError:
            return None
        _SOURCE_BYTES_CACHE[key] = data
    if end > len(data):
        return None
    try:
