...
```
owner.source_file  ->  stem  ->  sanitize + lowercase  ->  "{stem}_params" / "{Stem}Params"
```
...
- **Declaration site, not use site.** A parameter declared in a library file and consumed from
  ...
  renamed set.
- **`model.sysml` falls back to the declaring package.** It is the one stem that carries no
  identity of its own, and v6 staging rewrites the parent directory name, so the old
  parent-directory fallback shipped `inputs/root_0_params.json` from a relocated snapshot. The
  fallback is now the package that declares the owning root occurrence, which the graph carries
  and `InstanceGraph.validate` requires to be non-empty (`_declaring_package`,
  `elaboration/project.py:331`). A package-scoped owner in a `model.sysml` has no root
  occurrence to take an identity from and stops with `SI_RENDERING_COLLISION` under its own
  name rather than being given a guessed group.
- **The recorded label is a token, not a path.** `ParameterGroup.source_file` holds the
  normalized identity token the name was built from. It reaches generated bytes only as
  `"Parameters from {label}."` (`generation/entry_point.py:254`), and a path there would make
  that line read differently on the live route than on a relocated snapshot's.

Every entry point has a group by construction, so there is no orphan pass and no
`system_design` fallback bucket. An entry point that would render into two groups is refused
(`SI_RENDERING_COLLISION`) rather than assigned to one of them.

Group identity is pinned by value in `tests/conformance/test_exact_group_identity.py`, and the
live, in-place-v6, and relocated-v6 routes are asserted to produce byte-equal entry-point group
payloads — name, class, label, and every parameter.
