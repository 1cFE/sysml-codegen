...
exit points. Generated by `generate_pipeline_yaml()` in `generation/pipeline.py`
from `ComputationGraph` alone -- no raw extraction data needed. This is the "gold standard" generator
...
correct JSON input file. `_input_to_context()` in `generation/pipeline.py`
applies the prefix from
...
| REQ-PY-08 | An aliased channel's exit line SHALL render the modeler's instance-qualified name as its output filename (`{instance_path}__{alias_name}.json`); the exit **key** stays the canonical channel and the type token is unchanged (REQ-PY-06 holds), so simkit's key-is-a-channel check still passes | EXPOSE_PURE name surfacing (Item 11) | Aliased channel's exit line filename token equals `OutputAlias.output_filename`; key + type unchanged |
...
**Steps**:
1. Build `channel_field_map` from all module outputs
2. Build the alias filename map (`_build_alias_filename_map`, REQ-PY-08)
3. Build entry points context (`_build_entry_points`)
4. Build module contexts with input source formatting (`_build_module_contexts`)
5. Build exit points context (`_build_exit_points`)
6. Render `pipeline_yaml.jinja2`
...
### MODULE_OUTPUT Sources (in `_input_to_context`)
...
### ENTRY_POINT Sources (in `_input_to_context`)
...
|---------|------|--------|-----------|
| CalcUsage inputs | `"float"` | `_build_pipeline_module` | Compliant |
| Computed attribute inputs | `"float"` | `_build_computed_attr_module` | Compliant |
| Aggregation term inputs | `"float"` | `_build_aggregation_module` | Compliant |
| Aggregation multiplicity | `"float"` | `_build_aggregation_module` | Compliant |
...
`_build_exit_points`, which applies the override per channel (both in
`generation/pipeline.py`).
The map is policy at the call site; `_build_exit_points` just applies it. Template:
the exit-point `outputs:` block in `pipeline_yaml.jinja2` renders
