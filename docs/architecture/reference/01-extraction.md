...
| REQ-EXT-09 | Every `ConstraintUsage` — **including** its `assert` (`AssertConstraintUsage`) and `require`/plain subtypes, **excluding** `RequirementUsage` and its `satisfy` subtype — swept by `collect_constraint_manifest()` SHALL have a carrier in the constraint catalog era (Item 14 re-anchor, [28-constraint-lowering-and-catalog.md](28-constraint-lowering-and-catalog.md)): an eligible concrete entry, an explicit unassessed record, or a named requirement/satisfy exclusion — nothing silently absent. | Totality across every constraint-bearing fixture has no live proof: the test that carried it retired with the legacy stack, and CONSTRAINT-SEMANTICS Item 2 re-anchors the evidence (citation removed 2026-08-12, CONSTRAINT-SEMANTICS Item 1); `wi014_toy`'s `assert constraint affordable` (`toy_plant.sysml:51`) lands as an eligible catalog entry; a mutation check flips `include_subtypes=False` and confirms the assert is then MISSED from the sweep. See the [subtype-enumeration decision table](../../../../agentic-mbse/docs/subtype-enumeration-decision-table.md) (adapter docs, D4 home) for the per-call-site policy. |
