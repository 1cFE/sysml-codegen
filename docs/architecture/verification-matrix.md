...
| REQ ID | Requirement | Test File | Status |
|--------|-------------|-----------|--------|
| REQ-CA-01 | Every computed attribute SHALL lift to one exact graph node | `test_elaboration_computed_attrs.py::test_all_fifteen_computed_attributes_lift` | PASS |
| REQ-CA-02 | Computed attributes SHALL compile through expression IR | `tests/integration/test_computed_attributes_exact_route.py::test_every_formula_computed_attribute_becomes_an_auto_implemented_module` | PASS |
...
| REQ-CA-10 | Legacy tentative-chain classification | — *(classifier deleted in Phase 5 audit fix round)* | RETIRED |
| REQ-CA-04 | Legacy literal-classifier exclusion | — *(classifier deleted in Phase 5 audit fix round)* | RETIRED |
| REQ-CA-05 | Legacy UNRESOLVABLE classifier outcome | — *(classifier deleted in Phase 5 audit fix round)* | RETIRED |
