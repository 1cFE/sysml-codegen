...
#: Used only for the raw-elaborator arm below, which stops before the
#: ``source_file`` rewrite and therefore still carries raw parser paths. The
#: public routes themselves render one portable referent shape and are compared
#: unmasked — module provenance was the last masked field, and step 5 of the
#: Item-7 correction made the live route render it portably too. See
#: ``test_the_two_routes_agree_on_module_provenance``.
MASK = "<source-derived>"
...
def test_live_in_place_and_relocated_routes_have_one_graph(tmp_path: Path) -> None:
    ...
    assert _payload(live) == _payload(project(in_place)), (
        "the live route and the v6 route disagree on the projected computation graph"
    )
    assert _instance_fingerprint(elaborate_model_paths([FIXTURE])) == _instance_fingerprint(
        in_place
    ), "the live instance graph and the sealed graph disagree, sources included"
    assert _graph_digest_ignoring_sources(_live_instance_graph(FIXTURE)) == (
        _graph_digest_ignoring_sources(in_place)
    ), "the raw elaborator arm and the sealed graph disagree"


def test_the_two_routes_agree_on_module_provenance(tmp_path: Path) -> None:
    """Every source-derived field is equal by value across the two routes.

    Slice 3B made group identity model-derived and equal. Module ``source_file``
    was the residual: the live route recorded the caller's own checkout path
    (one spelling even carrying a leftover ``//`` URI prefix from the parser)
    while the v6 route recorded the portable referent, so every generated file
    with a ``SysML Source:`` comment differed between the routes. Step 5 of the
    Item-7 correction (audit-F4) made the live route render the same portable
    referent from its own model roots — without routing it through admission,
    so the arm independence this module depends on survives. Asserted by value,
    not merely by equality, so a regression to raw paths on *both* sides would
    still fail.
    """
    ...
    assert live["module_source_files"] == v6["module_source_files"] == {None, "root-0/model.sysml"}
