...
#: The one field the two routes still disagree on: a module's recorded
#: ``source_file``. The live builder records the caller's own checkout path; the
#: v6 route records the portable referent the sealed manifest names the file by.
#: It is masked for the structural comparison and then asserted by value, so the
#: divergence stays visible instead of being hashed over. Entry-point group
#: identity used to be masked here too and no longer is — Slice 3B made it
#: model-derived and therefore equal. See
#: ``test_the_two_routes_agree_on_group_identity_and_differ_only_on_module_provenance``.
MASK = "<source-derived>"
...
def _source_derived_naming(payload: dict) -> dict[str, set[str]]:
    return {
        ...
        "group_source_files": {group["source_file"] for group in payload["entry_point_groups"]},
    }
...
def test_the_two_routes_agree_on_group_identity_and_differ_only_on_module_provenance(
    tmp_path: Path,
) -> None:
    """Group identity is now equal on both routes; module provenance still is not.

    Slice 3B fixed the group half. ``_group_identity`` used to name a group after
    the source path, and for a ``model.sysml`` it fell back to the parent
    directory — the one input v6 staging rewrites — so a package generated from a
    v6 snapshot shipped ``inputs/root_0_params.json`` and a ``Root0Params`` schema
    class. The fallback is now the declaring package of the owning root
    occurrence, which both routes carry identically, so the group name, class, and
    recorded label are asserted equal here rather than masked.

    Module ``source_file`` is still route-dependent, and that is honest: the live
    route records the caller's own checkout path (one of them still carrying a
    leftover ``//`` URI prefix) while the v6 route records the portable referent.
    It reaches generated bytes only as a ``SysML Source:`` provenance comment.
    The v6 spelling is the one the design intends
    (``generation/stencils.py:243``); making the live route agree means routing it
    through admission, which would undo the arm independence this module depends
    on. Pinned by value here and carried to Slice 3E, not normalised away.
    """
    ...
    # The fix: one model-derived group identity, equal across both routes.
    assert live["group_names"] == v6["group_names"] == {"source_identity_mixed_consumers_params"}
    assert (
        live["group_class_names"]
        == v6["group_class_names"]
        == {"SourceIdentityMixedConsumersParams"}
    )
    assert (
        live["group_source_files"]
        == v6["group_source_files"]
        == {"source_identity_mixed_consumers"}
    )

    # The residual: module provenance still differs, by value on both sides.
