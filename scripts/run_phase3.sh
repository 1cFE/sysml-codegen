...
# Phase 3: Analysis Components — automated plan+build loop
#
...
#
# Usage:
#   ./scripts/run_phase3.sh          # run all 4 tasks
#   ./scripts/run_phase3.sh 3.1      # run only task 3.1
#   ./scripts/run_phase3.sh 3.2 3.3  # run tasks 3.2 and 3.3
...
LOG_DIR="$PROJECT_DIR/.project/logs/phase3"
...
TASKS=(
  "3.1|backtracker-conformance|DependencyBacktracker Conformance (C11)"
  "3.2|input-resolver|Input Resolver Spike (C12)"
  "3.3|parameter-group-deriver|ParameterGroupDeriver Conformance (C13)"
  "3.4|dual-resolution-consistency|Dual Resolution Consistency (X02)"
)
...
echo "═══════════════════════════════════════════════════════"
echo "  Phase 3: Analysis Components"
...
**Component**: \`$COMPONENT_NAME\` (plan at \`.project/active/$COMPONENT_NAME/plan.md\`)"
  ...
  if uv run pytest tests/ -x -q --tb=line 2>&1 | tail -5; then
    ...
    if [[ "${CONTINUE:-N}" != [yY] ]]; then
      echo "  Stopping. Fix issues and re-run: ./scripts/run_phase3.sh ${STEP_ID}"
      exit 1
    fi
  fi
